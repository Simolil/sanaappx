...
export default function MemoryConsole({ memories, onDeleteMemory }: MemoryConsoleProps) {
  ...
  return (
    <div id="memory-console" className="bg-[#fcfaf7] border border-[#e6dfd5] rounded-3xl p-6 relative overflow-hidden space-y-6">
      <div className="absolute top-0 right-0 w-32 h-32 bg-indigo-100/20 rounded-full blur-2xl -mr-10 -mt-10" />
      ...
      <div>
        ...
        <p className="font-sans text-xs text-[#6c5f54] mt-0.5">
      ...
      <div id="memory-concept-notice" className="p-3.5 bg-indigo-50/45 rounded-2xl border border-indigo-100 flex gap-3 text-xs text-indigo-950 font-sans leading-relaxed">
        <ShieldCheck className="w-5 h-5 text-indigo-700 shrink-0 mt-0.5" />
        <div>
          <span className="font-semibold text-indigo-900">Sovereign Data Storage:</span> You hold full control over your background memories. If Willow records something inaccurate, or if a trigger changes, click the trash icon below to erase it instantly.
        </div>
      </div>

      <div className="space-y-4">
        {memories.length === 0 ? (
          <div id="no-memories-empty" className="h-40 flex flex-col items-center justify-center border border-dashed border-[#e6dfd5] rounded-2xl bg-white/40">
            <Heart className="w-5 h-5 text-pink-500 opacity-40 mb-1.5 animate-bounce" />
            <span className="font-sans text-xs text-[#6c5f54] text-center px-4 leading-relaxed">
              Willow has not recorded any details yet.<br/>
              <span className="text-[11px] text-[#8c6239] font-medium">As you chat naturally about what upsets you, what calms you down, or who resides in your life, Willow will remember details!</span>
            </span>
          </div>
        ) : (
          (Object.keys(grouped) as MemoryCategory[]).map((cat) => {
            ...
            return (
              <div id={`memory-group-${cat}`} key={cat} className="space-y-1.5">
                <h4 className="font-sans font-semibold text-xs text-[#8c6239] uppercase tracking-wider flex items-center gap-1.5 px-1">
                  ...
                  <AnimatePresence mode="popLayout">
                    {list.map((mem) => (
                      <motion.div
                        ...
                        className={`p-3 rounded-xl border ${config.bg} ${config.border} flex items-center justify-between gap-3 shadow-none`}
                      >
                        <div className="flex-1 min-w-0">
                          <p className={`font-sans text-xs font-semibold ${config.text} leading-tight`}>
                            {mem.description}
                          </p>
                          <span className="text-[9px] text-[#6c5f54]/55 font-mono block mt-1">
                        ...
                        <button
                          ...
                          className="p-1 px-2.5 rounded-lg text-rose-800 hover:text-rose-950 hover:bg-rose-100/50 bg-transparent transition-all border border-transparent disabled:opacity-45"
